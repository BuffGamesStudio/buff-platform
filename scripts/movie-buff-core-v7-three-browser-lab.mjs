...
if (!expectedSha || !coreSha || !appUrl || !usersPath || !evidenceDir || !playwrightRoot) {
  throw new Error(
    "MOVIE_BUFF_EXPECTED_GIT_SHA, MOVIE_BUFF_CORE_SHA, MOVIE_BUFF_APP_URL, MOVIE_BUFF_LOCAL_USERS_OUTPUT, MOVIE_BUFF_EVIDENCE_DIR, and PLAYWRIGHT_PACKAGE_ROOT are required.",
  );
}
...
const evidence = {
  schemaVersion: 1,
  laboratory: "movie-buff-core-v7-three-browser",
...
try {
  for (let index = 0; index < players.length; index += 1) {
    ...
    page.on("pageerror", (error) => {
      evidence.pageErrors.push({ player: index + 1, message: error.message.slice(0, 500) });
    });
