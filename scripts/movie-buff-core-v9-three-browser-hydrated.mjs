...
if (!expectedSha || !compositionSha || !appUrl || !usersPath || !evidenceDir || !playwrightRoot) {
  throw new Error("Exact identity, local app, users, evidence directory, and Playwright root are required.");
}
...
const evidence = {
  schemaVersion: 3,
  laboratory: "ephemeral-three-browser-hydrated",
...
const pass = (name, details = {}) => evidence.checks.push({ name, classification: "PASS", details });
...
try {
  ...
  await Promise.all(
    pages.map(async (page, index) => {
      ...
      await page.waitForFunction(() => {
        ...
        return Boolean(targetForm && Object.keys(targetForm).some((key) => key.startsWith("__reactProps$")));
      }, null, { timeout: 60_000 });
  ...
  pass("three-authenticated-browser-sessions-after-hydration");
  ...
  assert.ok(roomIds.every(Boolean), "all three browsers must expose roomId");
  assert.equal(new Set(roomIds).size, 1, "all browsers must converge on one room");
  ...
  assert.equal(new Set(finalPaths).size, 1, "all browsers must share one runtime route");
  pass("shared-runtime-route", { path: finalPaths[0] });

  for (let index = 0; index < pages.length; index += 1) {
    const results = {};
    for (const text of forbidden) {
      results[text] = await pages[index].getByText(text, { exact: false }).count();
      assert.equal(results[text], 0, `forbidden manual phase control visible: ${text}`);
    }
    evidence.forbiddenControls[`player-${index + 1}`] = results;
    await pages[index].screenshot({ path: path.join(evidenceDir, `player-${index + 1}-runtime.png`), fullPage: true });
  }
