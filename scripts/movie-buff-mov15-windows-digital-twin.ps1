...
function Write-Classification {
  ...
  "MOV15_WINDOWS_DIGITAL_TWIN=$Value"
}
...
try {
  ...
  foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
      throw "Required MOV-15 file is missing from the expected working directory: $requiredPath"
    }
  }
  ...
  @(
    ...
    'combined_mov17_database_behavior=UNKNOWN'
  ...
  Invoke-Captured 'mov15-contract-tests' {
    node --test `
      tests/movie-buff-public-matchmaking-contract.test.mjs `
      tests/movie-buff-public-matchmaking-handoff.test.mjs
  }
