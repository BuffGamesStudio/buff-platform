...
const outputPath = path.resolve(
  process.env.MOVIE_BUFF_EVIDENCE_OUTPUT ??
    "movie-buff-vip-authority-adversarial-v2.json",
);
...
if (
  ...
  throw new Error(
    "NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY, SUPABASE_SERVICE_ROLE_KEY, MOVIE_BUFF_LOCAL_DATABASE_URL, MOVIE_BUFF_APP_URL, and MOVIE_BUFF_EXPECTED_GIT_SHA are required.",
  );
}
...
function createContext(participantIndexes, roundNumber) {
  ...
  const roomPlayerValues = participantIndexes
    .map((index, position) =>
  ...
  const matchPlayerValues = participantIndexes
    .map(
      (index) =>
        `(${quote(matchId)}::uuid,${quote(sessions[index].user.id)}::uuid)`,
    )
    .join(",");
...
function grantInventory(playerIndex, vipId, quantity = 2) {
  ...
  ownerSql(`
    ...
    values
      (${quote(id)}::uuid,${quote(sessions[playerIndex].user.id)}::uuid,${quote(vipId)}::uuid,${quantity});
  `);
  return id;
}
...
async function cleanup() {
  ...
  for (const vipId of [...createdDefinitions].reverse()) {
    try {
      ownerSql(`
        delete from public.movie_buff_vip_inventory where vip_id=${quote(vipId)}::uuid;
...
try {
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    const { data: authData, error: authError } = await clients[index].auth.signInWithPassword({
      email,
      password,
    });
  ...
  const activeMembershipCount = Number(
    ownerSql(`
      ...
      where player_id in (${sessions.map((session) => `${quote(session.user.id)}::uuid`).join(",")})
        and left_at is null;
    `),
  );
  assert.equal(activeMembershipCount, 0);
  ...
  const requiredIds = ownerSql(`
    select player_id::text from public.movie_buff_vip_round_required_players
    where round_id=${quote(windowRace.roundId)}::uuid order by player_id;
  `)
    .split("\n")
    .filter(Boolean);
  ...
  assert.match(
    String(contradictory.find((result) => result.status >= 400)?.payload?.error ?? ""),
    /different choice/i,
  );
  ...
  const quantityAfter = Number(
    ownerSql(`
      select quantity_remaining from public.movie_buff_vip_inventory
      where id=${quote(activationInventory)}::uuid;
    `),
  );
  assert.equal(quantityAfter, 1);
  const consumptionCount = Number(
    ownerSql(`
      select count(*) from public.movie_buff_vip_consumptions c
      join public.movie_buff_vip_round_locks l on l.id=c.lock_id
      where l.round_id=${quote(activationContext.roundId)}::uuid
        and l.player_id=${quote(sessions[0].user.id)}::uuid;
    `),
  );
