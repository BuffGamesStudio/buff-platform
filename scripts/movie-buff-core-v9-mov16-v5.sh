...
EXPECTED_SHA="${1:-}"
EXPECTED_TREE="${2:-}"
EVIDENCE_ROOT="${3:-${RUNNER_TEMP:-/tmp}/movie-buff-core-v9-mov16-v5-evidence}"
...
RUN_TOKEN="${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-1}-mov16-v5"
...
write_metadata() {
  {
    echo "lane=movie-buff-core-v9-mov16-activation-v5"
    ...
    echo "vip_activation=${CLASSIFICATION}"
    echo "vip_window_lock_release=UNKNOWN_ON_THIS_SHA"
...
finish() {
  ...
  if [[ -f "$RAW_ROOT/application.stdout.raw" ]]; then
    redact "$RAW_ROOT/application.stdout.raw" "$EVIDENCE_ROOT/application.stdout.txt"
  fi
  if [[ -f "$RAW_ROOT/application.stderr.raw" ]]; then
  ...
  printf 'MOVIE_BUFF_CORE_V9_MOV16_ACTIVATION_V5=%s\n' "$CLASSIFICATION"
  exit "$status"
}
trap 'finish $?' EXIT
...
PY
  ...
  run_step diagnostic-parse node --check "$SOURCE_ROOT/scripts/movie-buff-core-v9-mov16-activation-diagnostic.mjs" || { fail diagnostic-parse; return 1; }
  ...
  MOVIE_BUFF_EVIDENCE_COMMAND="node scripts/movie-buff-core-v9-mov16-activation-diagnostic.mjs" \
  MOVIE_BUFF_VIP_TEST_USERS="$users_json" \
  MOVIE_BUFF_EVIDENCE_OUTPUT="$EVIDENCE_ROOT/mov16-activation.json" \
    run_step mov16-activation node "$SOURCE_ROOT/scripts/movie-buff-core-v9-mov16-activation-diagnostic.mjs" || { fail mov16-activation; return 1; }

  jq -e '.classification == "PASS" and .exitCode == 0 and .cleanupFailure != true' \
    "$EVIDENCE_ROOT/mov16-activation.json" >/dev/null || { fail activation-evidence-classification; return 1; }
