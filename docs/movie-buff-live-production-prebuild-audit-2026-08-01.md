...
| Hidden-answer leakage | `roundService` returns `correctTitle` after submit/results; raw DB data contains answers. | Never publish raw round/answer rows to broadcast contracts. | [VERIFIED:LOCAL]+[INFERENCE] |
| VIP inventory leakage | Existing code mentions VIP/reward unlock text but no live VIP contract exists. | Contract marks inventory player-private only; broadcast receives eligibility tier only. | [VERIFIED:LOCAL]+[INFERENCE] |
...
| 3 | Event transport | [INFERENCE] Durable event log + Supabase Broadcast/Postgres Changes as notification. | Existing DB event log and Realtime refresh pattern. | Raw Realtime-only; client-to-OBS direct. | Add projection table/contracts later. | Snapshot rebuild on missed events. |
...
| 7 | Overlay transport | [INFERENCE] OBS browser source loads overlay page that subscribes to broadcast-safe updates. | OBS Browser Source is CEF-backed URL/local browser. | Static image-only; secrets in querystring. | Cache-busted overlay URL with session token. | Overlay reconnects and shows safe standby. |
