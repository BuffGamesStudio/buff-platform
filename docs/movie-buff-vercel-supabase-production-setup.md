...
It assumes the repo is already locally verified and the remaining blocker is
...
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase elevated server key for protected server/admin operations |
...
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase project dashboard API/settings area -> service role key |
...
- if `SUPABASE_SERVICE_ROLE_KEY` is missing, admin/runtime server operations will fail even if public pages load
...
- `SUPABASE_SERVICE_ROLE_KEY` must be stored only as a server environment variable in Vercel
