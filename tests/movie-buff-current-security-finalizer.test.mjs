...
const allowlistInsert = functions.match(
  /insert into movie_buff_authenticated_rpc_allowlist\(identity\) values([\s\S]*?);/,
)?.[1] ?? "";
const identities = [
  ...allowlistInsert.matchAll(/\('public\.[^']+'\)/g),
].map((match) => match[0]);

test("full candidate uses an exact 24-RPC authenticated allowlist", () => {
  assert.equal(identities.length, 24);
  assert.match(allowlistInsert, /get_movie_buff_active_leave_quote\(uuid\)/);
  assert.match(allowlistInsert, /confirm_movie_buff_active_leave\(uuid,text\)/);
  assert.doesNotMatch(
    allowlistInsert,
    /confirm_movie_buff_active_leave\(uuid,text,text\)/,
  );
  assert.match(functions, /grant execute on function %s to authenticated/);
  assert.doesNotMatch(
    functions,
    /grant execute on function[^;]+to anon/i,
  );
});
...
test("RLS event callback remains enabled and non-callable", () => {
  ...
  assert.doesNotMatch(autoRls, /drop event trigger/i);
  assert.doesNotMatch(autoRls, /alter event trigger[^;]+disable/i);
  assert.doesNotMatch(autoRls, /grant execute on function/i);
});

test("rollbacks preserve fail-closed containment", () => {
  assert.doesNotMatch(
    rollbacks,
    /grant execute[^;]+authenticated|grant select[^;]+authenticated/i,
  );
  assert.doesNotMatch(rollbacks, /drop table|truncate|delete from/i);
  assert.match(rollbacks, /force row level security/);
  assert.match(
    rollbacks,
    /event trigger remains enabled|ensure_rls event trigger must remain enabled/,
  );
});
