...
import { getGeneratedMediaPreview } from "@/lib/server/movieClipper";
...
async function buildGeneratedPreviewResponse(
  request: Request,
  mediaId: string,
  headOnly: boolean,
) {
  try {
    const summary =
      await getGeneratedMediaPreview(mediaId);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Generated clip preview could not be prepared.",
      },
      { status: 500 },
    );
  }
...
export async function GET(
  ...
  return buildGeneratedPreviewResponse(
    request,
    mediaId,
    false,
  );
