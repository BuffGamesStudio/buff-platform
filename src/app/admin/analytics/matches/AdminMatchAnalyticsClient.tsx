...
import type { MovieBuffMatchAnalytics } from "@/lib/server/movieBuffAnalyticsAdmin";

type AdminMatchAnalyticsClientProps = {
  initialAnalytics: MovieBuffMatchAnalytics;
...
export default function AdminMatchAnalyticsClient({
  initialAnalytics,
  initialError = null,
  initialLoaded = false,
}: AdminMatchAnalyticsClientProps) {
  const [analytics, setAnalytics] = useState<MovieBuffMatchAnalytics>(
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      const response = await adminFetch("/api/admin/analytics/matches", {
        cache: "no-store",
      });

      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Match analytics could not be loaded."),
        );
      }

      const payload = (await response.json()) as {
        analytics?: MovieBuffMatchAnalytics;
      };

      setAnalytics(payload.analytics ?? emptyAnalytics);
    } catch (error) {
      setErrorMessage(
        error instanceof Error ? error.message : "Match analytics could not be loaded.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="space-y-6 p-5 sm:p-8">
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          <div className="rounded-3xl border border-white/10 bg-zinc-950 p-6">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {loading ? (
                <p className="text-sm text-zinc-500">Loading event counts...</p>
              ) : (
          ...
          <div className="rounded-3xl border border-white/10 bg-zinc-950 p-6">
            <h2 className="text-xl font-black text-white">Recent room summaries</h2>

            <div className="mt-4 space-y-3">
              {loading ? (
                <p className="text-sm text-zinc-500">Loading room summaries...</p>
              ) : (
                analytics.roomSummaries.map((room) => (
                  <div
                    ...
                    className="rounded-2xl border border-white/10 bg-black/40 p-4"
                  >
                    <p className="text-sm font-black text-white">Room {room.roomId.slice(0, 8)}</p>
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-950">
          <div className="border-b border-white/10 px-6 py-5">
            <h2 className="text-xl font-black text-white">Recent event stream</h2>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[1000px] text-left text-sm">
              ...
              <tbody className="divide-y divide-white/5 text-zinc-200">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-6 text-zinc-500">
                  ...
                  analytics.recentEvents.map((eventRow) => (
                    <tr key={eventRow.id}>
                      ...
                      <td className="px-6 py-4 font-black text-white">{eventRow.event_type}</td>
                      <td className="px-6 py-4">{eventRow.room_id ? eventRow.room_id.slice(0, 8) : "—"}</td>
                      <td className="px-6 py-4">{eventRow.match_id ? eventRow.match_id.slice(0, 8) : "—"}</td>
                      <td className="px-6 py-4">{eventRow.round_id ? eventRow.round_id.slice(0, 8) : "—"}</td>
