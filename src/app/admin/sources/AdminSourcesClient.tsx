...
import type { ContentSourceSummary } from "@/lib/server/contentSources";

type AdminSourcesClientProps = {
  initialSources: ContentSourceSummary[];
...
export default function AdminSourcesClient({
  ...
  const [sources, setSources] = useState<ContentSourceSummary[]>(initialSources);
  ...
  const loadSources = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await getApiErrorMessage(response, "Source registry data could not be loaded."),
        );
      }
  ...
  const approvedSources = useMemo(
    () => sources.filter((source) => source.clipIngestSuitability === "approved"),
    [sources],
  );
  const autoIngestSources = useMemo(
    () => sources.filter((source) => source.autoIngestAllowed),
    [sources],
  );
      ...
      <div className="space-y-8 p-5 sm:p-8">
        ...
        <section className="grid gap-4 xl:grid-cols-3">
          <div className="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5 text-emerald-100">
            <p className="text-sm font-bold uppercase tracking-[0.18em]">Approved now</p>

            <p className="mt-2 text-sm leading-7">
              These sources are acceptable for Movie Buff clip intake only when each
              title still passes item-level rights validation.
            </p>
          </div>
                  ...
                  sources.map((source) => (
                    <tr key={source.id}>
                      ...
                      <td className="px-6 py-5 align-top text-xs leading-6 text-zinc-400">
                        ...
                        <p>Success: {formatDateTime(source.lastSuccessfulIngestAt)}</p>
                      </td>
                    </tr>
                  ))
                )}
        ...
        <section className="rounded-3xl border border-white/10 bg-zinc-950 p-6">
          <h2 className="text-xl font-black text-white">Launch policy reminders</h2>
          ...
          <ul className="mt-4 space-y-3 text-sm leading-7 text-zinc-400">
            ...
            <li>
              Internet availability is not enough. Every gameplay candidate still
              needs item-level rights confirmation before live clip use.
            </li>
            <li>
