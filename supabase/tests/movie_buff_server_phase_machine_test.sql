...
select ok(
  position(
    ...
    in pg_get_functiondef('public.movie_buff_activate_ready_busters(uuid)'::regprocedure)
  ) > 0,
  'Buster activation is limited to authoritative board-select and results boundaries'
);
select ok(
  position(
    'v_state.phase = ''vip_lock'''
    in pg_get_functiondef('public.movie_buff_activate_ready_busters(uuid)'::regprocedure)
  ) = 0,
  'Buster activation is never authorized while the current phase is vip_lock'
);
