-- Read-only post-chain verifier for Movie Buff successor expected-state manifest v3.
-- Manifest path:
...
insert into expected_critical_functions values
  ...
  ('public.join_movie_buff_room(text)','search_path=pg_catalog',true,true),
...
) as movie_buff_successor_manifest_v3_verification;
