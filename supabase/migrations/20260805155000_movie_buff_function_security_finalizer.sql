-- Movie Buff catalog-driven function ownership, search_path, and ACL finalizer.
-- Apply only after the current MOV-15/MOV-16/MOV-17 migrations, including
-- 20260804073300_movie_buff_vip_deadline_finalize.sql,
-- 20260804083000..20260804083700 server phase-machine and active-leave migrations.
...
-- Every allowlisted browser RPC is a required contract. Missing identities abort.
do $contract$
declare
  v_identity text;
begin
  ...
  if pg_catalog.to_regprocedure(
    'public.finalize_movie_buff_vip_round_window(uuid,uuid,timestamp with time zone)'
  ) is null then
...
-- Reset every Movie Buff function to a closed baseline, normalize ownership,
-- and preserve an already-stricter pg_catalog-only path where present.
do $harden$
declare
  v_function record;
  v_identity text;
...
-- Re-open only the caller-scoped browser contract. Shared phase internals,
-- legacy manual transition RPCs, and the VIP deadline finalizer remain service-only.
do $browser$
declare
  v_identity text;
begin
  for v_identity in select identity from movie_buff_authenticated_rpc_allowlist loop
    execute pg_catalog.format(
      ...
      pg_catalog.to_regprocedure(v_identity)::pg_catalog.regprocedure
    );
  end loop;
end;
$browser$;

-- Catalog assertions make the migration fail closed.
do $verify$
declare
  v_bad text;
begin
  ...
  where n.nspname = 'public'
    and (
      p.proname like '%movie_buff%'
      or p.proname in ('set_updated_at','normalize_movie_answer','handle_new_user')
    )
    and (
      owner_role.rolname <> 'postgres'
      or not coalesce(p.proconfig, '{}'::text[]) && array[
      ...
      or (
        ...
        <> exists (
          select 1
          from movie_buff_authenticated_rpc_allowlist a
  ...
  if v_bad is not null then
    raise exception 'Movie Buff function contract failed for %', v_bad;
  end if;
