-- MOV-17 follow-up hardening after static review.
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'movie-buff-phase-bootstrap|' || v_match.id::text,
      0
    )
  );
...
begin
  ...
  exception
    when others then
      if sqlerrm not ilike '%snapshot entry not found%'
         and sqlerrm not ilike '%window not found%' then
        raise;
      end if;
  end;
end;
...
begin
  ...
  if p_expected_version is null or p_expected_version <= 0 then
    raise exception 'A positive expected phase version is required.';
  end if;
...
drop trigger if exists movie_buff_answers_require_authoritative_phase
  on public.answers;
create trigger movie_buff_answers_require_authoritative_phase
before insert on public.answers
for each row execute function public.movie_buff_guard_authoritative_answer_phase();
...
alter function public.movie_buff_guard_authoritative_answer_phase() owner to postgres;

revoke all on function public.movie_buff_guard_authoritative_answer_phase()
  from public, anon, authenticated, service_role;

-- Manual client-driven round advancement is incompatible with the canonical
-- phase row. Trusted service continuity remains for rollback/containment tools.
