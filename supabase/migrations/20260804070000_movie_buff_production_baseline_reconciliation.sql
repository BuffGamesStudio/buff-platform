...
begin
  ...
  if exists (
    ...
    where t.relname = 'match_rounds'
      ...
      and pg_catalog.pg_get_constraintdef(c.oid, true) <> 'CHECK (hint_penalty_seconds >= 0 AND hint_penalty_seconds <= 10)'
  ) then
  ...
  if to_regprocedure('public.is_movie_buff_room_member(uuid)') is not null then
    raise exception 'Baseline mismatch: public.is_movie_buff_room_member(uuid) already exists.';
  end if;

  if to_regprocedure('public.is_movie_buff_match_member(uuid)') is not null then
    raise exception 'Baseline mismatch: public.is_movie_buff_match_member(uuid) already exists.';
  end if;

  if to_regprocedure('public.is_movie_buff_round_member(uuid)') is not null then
    raise exception 'Baseline mismatch: public.is_movie_buff_round_member(uuid) already exists.';
  end if;

  if to_regprocedure('public.is_buff_content_manager()') is not null then
    raise exception 'Baseline mismatch: public.is_buff_content_manager() already exists.';
  end if;
end;
$preflight$;
...
create function public.is_movie_buff_match_member(
  p_match_id uuid
)
returns boolean
language sql
...
create function public.is_movie_buff_round_member(
  p_round_id uuid
)
returns boolean
language sql
...
create function public.is_buff_content_manager()
returns boolean
language sql
stable
security definer
