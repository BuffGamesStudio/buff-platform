-- Rollback for 20260807010000_movie_buff_production_security_manifest_reconciliation.sql.
--
-- Restores the exact pre-070100 posture produced by the sealed RC chain after
-- 20260805161000. This is a recovery artifact, not the desired final security
-- state. Execute only in reverse dependency order after later migrations have
-- been removed/contained.
...
begin
  foreach v_identity in array array[
    ...
    if pg_catalog.to_regprocedure(v_identity) is null then
      raise exception 'Rollback prerequisite function is absent: %', v_identity;
    end if;
  end loop;

  foreach v_table in array v_tables loop
    if pg_catalog.to_regclass(pg_catalog.format('public.%I', v_table)) is null then
      raise exception 'Rollback prerequisite table is absent: public.%', v_table;
    end if;
  end loop;
end;
$preflight$;

-- Restore the post-20260805155000 browser/function ACL posture for these three
-- helpers. The later 070100 migration is what removes authenticated EXECUTE.
...
-- Restore the pre-070100 media-ready implementation while retaining the
-- hardened owner/search_path/ACL posture produced by the preceding finalizer.
...
-- Restore the pre-070100 FORCE-RLS topology produced by the exact RC functional
-- chain: five server ledgers forced, ten RLS-enabled but not forced. Browser
-- grants are not restored because the preceding chain already keeps these
-- internal tables non-browser-readable.
...
-- Post-rollback topology assertions. These deliberately assert the prior state,
-- not manifest-v2 PASS.
do $verify$
declare
  v_table text;
  v_forced_tables constant text[] := array[
...
begin
  if not pg_catalog.has_function_privilege(
    ...
    raise exception 'Rollback verification failed: helper browser ACL was not restored.';
  end if;
  ...
  foreach v_table in array v_forced_tables loop
    if not (
      ...
      raise exception 'Rollback verification failed for forced table public.%', v_table;
    end if;
  end loop;
  ...
  foreach v_table in array v_not_forced_tables loop
    if not (
      ...
      raise exception 'Rollback verification failed for non-forced table public.%', v_table;
    end if;
  end loop;
end;
$verify$;
